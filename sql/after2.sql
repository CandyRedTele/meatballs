...
BEGIN
    ...
    INSERT INTO update_balance_after_bill_log (msg, mitem_id, b_id, f_id, price, 
     ...
     -- SET @quantity_old = (SELECT quantity FROM facilityStock WHERE f_id = @location);
 
     CREATE TEMPORARY TABLE IF NOT EXISTS table1 AS  (SELECT sku, amount FROM ingredients  WHERE mitem_id = NEW.mitem_id);
     
     update facilityStock 
     SET quantity = (select amount from table1 where facilityStock.sku = table1.sku AND facilityStock.f_id = @location)
     WHERE facilityStock.f_id = @location;
